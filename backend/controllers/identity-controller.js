...
const loginUser = async (req, res) => {
  ...
  try {
    ...
    // Allow Google-only users to log in with any password? NO!
// Better: if user has no password → force Google login
if (!user.password) {
  return res.status(400).json({
    ...
    // GENERATE TOKENS FIRST (sets cookies)
    const { accessToken, refreshToken } = await generateToken(user, res);
...
const refreshTokenUser = async (req, res) => {
  ...
  // Generate new tokens (sets cookies)
